  ...
  #updateVisibleSlides() {
    ...
    scheduler.schedule(() => {
      ...
      slides.forEach((slide, index) => {
        // The first slide is always visible
        if (index === 0) return;
...
/**
 * @typedef {Object} ScrollBarRefs
 * @property {HTMLElement} track
 * @property {HTMLElement} thumb
 */

/**
 * Custom scrollbar for slideshow component
 * @extends {Component<ScrollBarRefs>}
 */
export class SlideshowScrollBar extends Component {
  requiredRefs = ['track', 'thumb'];
  ...
  /**
   * Setup scroll synchronization with slideshow
   */
  #setupScrollSync() {
    if (!this.#slideshow) return;

    const scroller = this.#slideshow.refs.scroller;
    if (!scroller) return;

    scroller.addEventListener('scroll', this.#handleScroll);

    this.#slideshow.addEventListener('slideshow:select', this.#handleSlideChange);
  ...
  /**
   * Setup user interactions (click only)
   */
  #setupInteractions() {
  ...
  #removeListeners() {
    ...
    this.#slideshow.removeEventListener('slideshow:select', this.#handleSlideChange);

    const { track } = this.refs;
    if (track) track.removeEventListener('pointerdown', this.#handlePointerDown);
  }

  /**
   * Handle scroll event from slideshow
   */
  #handleScroll = () => {
    this.#updateScrollbar();
  };
  ...
  #updateScrollbar() {
    ...
    const thumbPosition = maxThumbPosition * scrollPercentage;
  ...
  /**
   * Handle pointer down on track or thumb - drag to scroll
  ...
  #handlePointerDown = (event) => {
    ...
    /**
     * Update scroll position based on pointer position
    ...
    const updateScrollPosition = (e) => {
      ...
      const thumbCenterPosition = Math.max(thumbWidth / 2, Math.min(clickX, trackWidth - thumbWidth / 2));
      const thumbPosition = thumbCenterPosition - thumbWidth / 2;
      ...
      scroller.scrollLeft = maxScroll * scrollPercentage;
    };

    // Update on initial click
    updateScrollPosition(event);

    // Setup capture for smooth dragging
...
customElements.define('slideshow-scroll-bar', SlideshowScrollBar);
